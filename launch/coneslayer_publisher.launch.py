...
def generate_launch_description():
    package_path = get_package_share_directory('ros_coneslayer')

    default_rviz = os.path.join(package_path,
                                'rviz', 'pointCloud.rviz')
    urdf_launch_dir = os.path.join(get_package_share_directory('depthai_descriptions'), 'launch')
    default_resources_path = os.path.join(package_path,
                                'resources')
    ...
    configName         = LaunchConfiguration('configName', default = "x")
    ...
    declare_configName_cmd = DeclareLaunchArgument(
        'configName',
        default_value=configName,
        description='Path to the object detection config.json needed for detection')
    ...
    coneslayer_node = launch_ros.actions.Node(
        package='ros_coneslayer', executable='coneslayer_node',
        output='screen',
    ...
    ld.add_action(declare_configName_cmd)
    ld.add_action(declare_resourceBaseFolder_cmd)
    ld.add_action(urdf_launch)

    ld.add_action(coneslayer_node)
